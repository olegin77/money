...
export class RecurringTransactionsProcessor {
  ...
  async handleProcessRecurring(_job: Job): Promise<void> {
    this.logger.log('Processing recurring transactions job...');
  ...
  private async processRecurringExpenses(today: Date, dateStr: string): Promise<number> {
    ...
    for (const expense of recurringExpenses) {
      ...
      try {
        ...
        this.logger.warn(`Failed to process recurring expense ${expense.id}: ${error}`);
      }
    }
  ...
  private async processRecurringIncome(today: Date, dateStr: string): Promise<number> {
    ...
    for (const income of recurringIncome) {
      ...
      try {
        ...
        this.logger.warn(`Failed to process recurring income ${income.id}: ${error}`);
      }
    }
